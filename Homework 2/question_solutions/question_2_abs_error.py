...
def computeAbsError() -> np.array:
    ...
    for step in steps:
        # Constructing tree
        candidate_tree = fe621.tree_pricing.trinomial.AdditiveTree(
    ...
    # Saving to CSV
    abs_error_df.to_csv(
        'Homework 2/bin/trinomial_tree_abs_error.csv', index=False)
...
def plotAbsError(steps: np.array, abs_error: np.array):
    # Equation label
    eq_label = r'$\epsilon_N=\left|P^{BSM}(\cdot)-P^{TTree}_N(\cdot)\right|$'
    ...
    # Save to file
    plt.savefig(fname='Homework 2/bin/trinomial_abs_error_plot.png')
...
if __name__ == '__main__':
    ...
    # abs_error = pd.read_csv('Homework 2/bin/trinomial_tree_abs_error.csv',
